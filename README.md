...
* 替换单调的红色 favicon 为动态变色的菱形图标。
* 服务器状态请求增加 10 分钟缓存，减少网络开销。
* 在加载 VPS 页面时，IP 国家查询改为异步执行，避免阻塞渲染。
* 通过打包和压缩静态资源优化加载性能。
* 部署脚本在完成后显示最新提交哈希，便于验证运行版本。
* 获取 VPS 列表数据时添加加载动画，提升用户反馈。
* 添加一键复制 Markdown 帖子内容，便于分享。
* 支持可选端口的 TCP ping，并在展示 IP 时自动隐藏端口号。
* 修复编辑页面剩余价值计算问题，统一各页面的最终价格公式。
